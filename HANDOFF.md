...
### 지역 검색 대응 ("대전 PCB업체", "대전 전장업체" 등)
- 기본 title/description에 대전·PCB 설계·전장 설계 반영 (`layout.tsx`)
- JSON-LD: `LocalBusiness`로 변경, 주소 시/구 뒤바뀜 수정, 우편번호 34036(확인됨), `hasMap`, `areaServed`(대전·세종·충남·충북), `serviceType` → `knowsAbout`
- 홈 hero 배지·H1·소개문·태그, 회사소개 설명·hero, 푸터 소개 문구에 대전·PCB·전장 반영
- 서비스 페이지: title "PCB 설계·전장 설계 서비스", H1 변경, **"대전 PCB 설계·전장 설계 제작" 설명 섹션 추가**(연혁의 실제 프로젝트만 근거로 작성 — 사용자 검토 필요)
- 앵커 수정: 서비스 카드 id `hardware/software/integration/automotive`, 회사소개 연혁 `id="history"`

### 회사 소개 사이트 적합성 분석 후 정리 (사용자 선택: 1·3·5·6·7)
- **1 가짜 포트폴리오·통계**: 연혁에 없던 4건(PLC 공장자동화, 의료기기 펌웨어 "인증 획득", BMS, 2024 EV WSS) 삭제. 포트폴리오를 연혁 기반 실제 12건으로 재구성(설명은 연혁 문구 수준, 필터 탭 제거 → 서버 컴포넌트 + metadata). 근거 없는 "99% / 50+ / 100+" 전부 삭제
- **3 대표 실적 재선정**: 홈 "대표 프로젝트"를 위성 6호 보드·잠수함 시뮬레이터 패널·LYNX 해군 패널·복강경 장비·경찰 VR·드론 보드 6건으로 교체 (모두 기존 연혁에 공개된 항목)
- **5 진행 방식**: 서비스 페이지에 "진행 방식" 5단계(상담 → 회로설계·아트웍 → 시제품·펌웨어 → 양산·검사 지그 → 전장 설치·정비) + 단계별 산출물 추가. **기판 층수·양산 규모·설계 툴·장비는 정보 없어 미기재**
- **6 구조 정리**: 통계 섹션은 홈 1곳만(2015 설립 / 10년+ / 20+ 수행 프로젝트 — 연혁 22건 기준), 회사소개·서비스·포트폴리오 통계 삭제. 문의 페이지 지도 제거 → "지도 보기" 링크로 오시는 길 연결. 오시는 길 연락처 카드 문구 수정(이메일 표기). 이모지 전부 `src/components/ui/Icon.tsx` 선형 SVG 아이콘으로 교체
- **7 마무리**: 한글 웹폰트 Noto Sans KR(next/font, Geist 제거), 한국어 404(`not-found.tsx`), 공유 미리보기 이미지(`opengraph-image.tsx`, 로고+gspls.kr, 빌드 시 생성). 없는 파일 참조(og-image.png, apple-touch-icon.png) 제거
...
- [ ] **구글 비즈니스 프로필 등록** (business.google.com) — "대전 ○○업체" 같은 지역 검색의 지도 영역 노출에 가장 효과 큼. 업종·주소·웹사이트·사진, 사업장 인증, 거래처 리뷰
- [ ] **네이버 스마트플레이스 등록** — 네이버 지역 검색용
- [ ] 거래처·협회·업체 목록 사이트에 gspls.kr 링크 확보

---

## 3. 남은 작업 — 사용자 결정 필요

- [ ] **사진 확보** (적합성 분석 2번): 보드·장비·시뮬레이터·작업 장면·사무실 5~10장 → 홈·서비스·포트폴리오에 배치. 현재 사이트에 사진 0장
- [ ] **회사 개요·대표 인사말** (적합성 분석 4번): 대표자, 인원, 인증·등록, 보유 장비 정보 필요
- [ ] **서비스 대응 범위** (적합성 분석 5번 잔여): 기판 층수, 양산 규모, 설계 툴(Altium/OrCAD 등), 측정 장비 → `work/page.tsx` 진행 방식 섹션에 추가
- [ ] **홈 배경 영상** 내용 확인: 자사 장비·작업 장면인지, 범용 소스인지 (범용이면 교체·삭제 검토)
- [ ] **"1영업일 내" 잔여 2곳**: 홈 바로가기 카드(`page.tsx`), 문의 페이지 metadata description(`contact/page.tsx`). 사용자 답변 대기
...
- [x] **포트폴리오 가짜 프로젝트 4개** — 삭제 완료, 연혁 기반 12건으로 재구성 (위 "적합성 분석 후 정리" 참고)
  - 남은 보강: 각 프로젝트 설명이 연혁 한 줄 수준. 사용자가 세부 내용(역할, 규모, 결과)을 주면 보강
  - 고객사명 "A사~D사"는 익명 처리로 유지
...
- [x] **통계 숫자** — 정리 완료. 홈 1곳만 남기고(2015 설립 / 10년+ / 20+ 수행 프로젝트) 나머지 삭제. 실제 프로젝트 수가 더 많으면 `page.tsx`의 `stats` 값 수정
...
- [ ] **확인 요청**
  - [x] 우편번호: `34134` → `34036`으로 수정 완료 (post119에서 배울1로 277 = 34036 확인)
...
- [x] **없는 파일 참조** — 참조 삭제, `opengraph-image.tsx`로 공유 이미지 생성. (디자인된 이미지가 생기면 `src/app/opengraph-image.png`로 교체 가능)
  - 참고: `public/gsplplus_logo.png`는 확장자만 png이고 실제 형식은 JPEG(투명 배경 없음). 동작에는 문제 없으나 투명 로고가 필요하면 진짜 PNG로 교체
- [x] **404 페이지** — `src/app/not-found.tsx` 추가
- [x] **오시는 길 `infoCards`** — 문구 수정 완료. 문의 페이지 지도는 제거하고 "지도 보기" 링크로 연결
- [x] **전화 흔적** — 아이콘 교체 완료. "무료 상담 신청" 문구(홈 hero 카드·CTA)는 그대로 둠 — 바꾸려면 `page.tsx` 2곳
- [x] **헤더 중복**: 데스크톱 헤더 이메일 링크 삭제 완료 ("문의하기" 버튼만 유지, 모바일 메뉴의 이메일은 유지)
- [ ] **lint** (`npx eslint src`)
  - ~~`portfolio/page.tsx`: 미사용 `Link` import~~ (재작성으로 해결)
